// Description: realtime dashboard mini-site work case, all text from en.json
// Author: Pinchas
...
const c = content.workPages.realtime
...
export default function RealtimePage() {
  ...
  const [activeNav, setActiveNav] = useState(c.navItems[0])

  const sidebarStats = c.stats.map((s) => ({
    label: s.label,
    value: 'value' in s ? s.value : `${latency}${'valueSuffix' in s ? s.valueSuffix : ''}`,
  }))

  const metricValues = c.metricCards.map((m) => ({
    label: m.label,
    value: 'value' in m ? m.value : `${latency}${'valueSuffix' in m ? m.valueSuffix : ''}`,
    delta: m.delta,
  }))

  const eventLog = [
    { t: `00:${String(Math.floor(latency / 100)).padStart(2, '0')}`, e: 'TRAINEE_ACTION', u: c.users[1].name, d: 'completed drill #7', col: '#22c55e' },
    { t: '00:04', e: 'SOCKET_TICK', u: 'server', d: `tick #${tick} dispatched to 247 clients`, col: '#0ea5e9' },
    { t: '00:09', e: 'SESSION_START', u: c.users[0].name, d: "opened session 'Sprint training'", col: '#a78bfa' },
    { t: '00:11', e: 'TRAINEE_ACTION', u: c.users[3].name, d: 'paused at 00:02:34', col: '#22c55e' },
    { t: '00:14', e: 'SOCKET_TICK', u: 'server', d: `tick #${tick - 1} dispatched to 247 clients`, col: '#0ea5e9' },
    { t: '00:18', e: 'AUTH_REFRESH', u: c.users[2].name, d: 'JWT refreshed silently', col: '#64748b' },
  ]
    ...
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", background: '#070d1a', color: '#e2e8f0', minHeight: '100vh', overflowX: 'hidden' }}>
      <nav style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', background: '#0b1221', padding: '0 24px', display: 'flex', alignItems: 'center', height: 56, gap: 32, position: 'sticky', top: 0, zIndex: 40 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginRight: 'auto' }}>
          ...
          <span style={{ fontWeight: 600, fontSize: 15, color: '#f1f5f9', letterSpacing: '-0.01em' }}>{c.appName}</span>
          <span style={{ fontSize: 11, color: '#64748b', marginLeft: 4, fontWeight: 400 }}>{c.appSubtitle}</span>
        </div>
        {c.navItems.map((n) => (
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, color: '#94a3b8' }}>{c.wsStatus}</span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 0, height: 'calc(100vh - 56px)' }}>
        <aside style={{ borderRight: '1px solid rgba(255,255,255,0.07)', background: '#0b1221', padding: '20px 0', overflowY: 'auto' }}>
          <div style={{ padding: '0 16px 12px', fontSize: 10, fontWeight: 600, color: '#475569', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{c.connectedNowLabel}</div>
          {c.users.map((u) => (
          ...
          <div style={{ padding: '0 16px 8px', fontSize: 10, fontWeight: 600, color: '#475569', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{c.statsLabel}</div>
          {sidebarStats.map((s) => (
        ...
        <main style={{ padding: 24, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <h1 style={{ fontSize: 22, fontWeight: 700, color: '#f1f5f9', margin: 0, letterSpacing: '-0.02em' }}>{c.pageTitle}</h1>
              <p style={{ fontSize: 13, color: '#64748b', margin: '4px 0 0' }}>{c.pageMetaPrefix}{tick}</p>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button style={{ fontSize: 12, padding: '8px 14px', borderRadius: 6, background: 'rgba(14,165,233,0.12)', color: '#38bdf8', border: '1px solid rgba(14,165,233,0.25)', cursor: 'pointer' }}>{c.exportBtn}</button>
              <button style={{ fontSize: 12, padding: '8px 14px', borderRadius: 6, background: '#0ea5e9', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 600 }}>{c.newSessionBtn}</button>
            </div>
          </div>
          ...
          <div style={{ background: '#0b1221', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '16px 18px' }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#e2e8f0', marginBottom: 14 }}>{c.eventThroughputLabel}</div>
            ...
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, lineHeight: 1.8 }}>
              {eventLog.map((ev, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, padding: '3px 0', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                  ...
                  <span style={{ color: ev.col, minWidth: 120 }}>{ev.e}</span>
